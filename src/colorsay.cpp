...
using namespace std;
colorsay::ColorSayPlugin g_pPlugin;
...
namespace colorsay {
    ...
    bool ColorSayPlugin::Load(CreateInterfaceFn interfaceFactory, CreateInterfaceFn gameServerFactory) {
        ...
        if(!Globals::SetGlobals(interfaceFactory, gameServerFactory)) {
    ...
    PLUGIN_RESULT ColorSayPlugin::ClientCommand(edict_t *pEdict, const CCommand &args) {
        if(strcasecmp("colorsay", args.Arg(0)))
            return PLUGIN_CONTINUE;
        if(args.ArgC() == 1) {
            console::println(pEdict, "Usage: colorsay <command>");
            return PLUGIN_STOP;
        }
        if(!clientcommands::exists(args.Arg(1))) {
            ostringstream ss;
        ...
        return clientcommands::invoke(pEdict, stl_argv.front(), stl_args, stl_argv);
    }
